"""
Demo data fixtures — returned when DB is unavailable or user is a demo account.
This lets the UI be fully walked through without a real database connection.
"""
...
DEMO_ENROLLMENTS_ADMIN = DEMO_ENROLLMENTS_INSTRUCTOR + [
    {
        ...
        "course_id": 3,
        "course_title": "Database Design & SQL",
...
# IDs that are considered "demo" accounts (no DB needed)
DEMO_USER_IDS = {9001, 9002, 9003}


def is_demo_identity(identity: dict) -> bool:
    """Returns True if the JWT identity belongs to a hardcoded demo account."""
    return identity.get("user_id") in DEMO_USER_IDS
